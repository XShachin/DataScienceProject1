...
\{CSCI 4360 -- Data Science ML II}
\author{Abrar Chowdhury, Gracyn Kim, Shachin Chakraborty, and Wren Nicol}
\date{Fall 2026}
...
Self-Noise. For each dataset, we check string-valued variables,
missing values, and potential outliers; summarize the variables; examine
Pearson correlations; select the two predictors with the strongest absolute
correlations with the response; and fit separate simple linear regression
models in both Python \texttt{statsmodels} and ScalaTion.

Potential outliers were identified using the $1.5\times IQR$ rule. These
outliers flagged by this rule were not automatically removed; the decision
to keep or remove them was based on whether there was evidence that the value
...
Ten potential outliers were found for horsepower and eleven for
acceleration. No potential outliers were identified for MPG, cylinders,
displacement, weight, or model year. The flagged outliers were kept because altough
the IQR rule flagged them, we did not deem the values to be implausible.
